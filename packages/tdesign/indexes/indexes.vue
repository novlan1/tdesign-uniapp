  ...
  >
    ...
    >
      <view
        ...
        :key="index"
        :class="_.cls(classPrefix + '__sidebar-item', [['active', current === item]]) + ' ' + tClassSidebarItem"
        :data-index="index"
        @click="() => onClick(item, index)"
      >
        <view
          aria-role="button"
          :aria-label="current === item ? '已选中' + item : ''"
        >
          {{ getFirstCharacter(item) }}
        </view>

        <view
          v-if="showTips && current === item"
    ...
    onAnchorTouch: throttle(function (e) {
      const getAnchorIndex = (clientY) => {
        ...
        if (offsetY > this.sidebar.height) {
          return this._indexList.length - 1;
        }
        ...
        this.$children.forEach((child, index) => {
          if (index === curIndex) {
            ...
            if (anchorStyle !== child.anchorStyle || sticky !== child.sticky) {
              child.sticky = sticky;
              child.active = true;
              child.style = `height: ${curGroup.height}px`;
