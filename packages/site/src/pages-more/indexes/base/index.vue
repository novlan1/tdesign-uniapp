<template>
    <view>
        ...
        <t-indexes :current="defaultCurrent" :index-list="indexList" :sticky-offset="stickyOffset" @select="onSelect" @change="onChange">
            <block v-for="(item, index) in list" :key="index">
                <t-indexes-anchor :index="item.index" />

                <t-cell-group>
                    <t-cell :title="city" aria-role="button" v-for="(city, index1) in item.children" :key="index1"></t-cell>
                </t-cell-group>
            </block>
        </t-indexes>
    </view>
