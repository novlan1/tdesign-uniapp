    ...
    <view>
        <block v-for="(item, i) in nodes" :key="i">
            ...
            <block v-else-if="item.type === 'list'">
                <view :class="classPrefix + '-list ' + (item.ordered ? classPrefix + '-list__decimal' : '')" :data-type="item.ordered">
            ...
            <block v-else-if="item.type === 'blockquote'">
                <view :class="classPrefix + '-blockquote'"><chat-markdown-node :nodes="item.tokens" /></view>
            </block>
            ...
            <block v-else-if="item.type === 'text'">
                <view :class="classPrefix + '-text ' + classPrefix + '-inline'" :data-raw="item.raw">
                    <block v-if="item.tokens && item.tokens.length">
                        <chat-markdown-node :nodes="item.tokens" />
                    </block>
                    <block v-else>{{ '' + item.raw + '' }}</block>
                </view>
            </block>
            ...
            <block v-else-if="item.type === 'link'">
                <view :class="classPrefix + '-link ' + classPrefix + '-inline'" :data-index="i" @tap="linkClick">
                    <block v-if="item.tokens && item.tokens.length">
                        <chat-markdown-node :nodes="item.tokens" />
                    </block>
                </view>
            ...
            <block v-else-if="item.type === 'ref'">
                <view :class="classPrefix + '-ref ' + classPrefix + '-inline'">
                    <text :class="classPrefix + '-ref-txt'">{{ '' + item.text + '' }}</text>
                </view>
            </block>

            <block v-else-if="item.type === 'space'">
                <view :class="classPrefix + '-space'"></view>
            </block>

            <block v-else-if="item.type === 'br'">
                <view :class="classPrefix + '-br'"></view>
            </block>

            <block v-else-if="item.type === 'hr'">
                <view :class="classPrefix + '-hr'"></view>
            </block>

            <block v-else-if="item.type === 'codespan'">
                <view :class="classPrefix + '-codespan ' + classPrefix + '-inline'" :data-type="item.type">
                    {{ '' + (item.text || item.raw) + '' }}
                </view>
            </block>

            <block v-else>
                <view :class="classPrefix + '-raw ' + classPrefix + '-inline'" :data-type="item.type">
                    {{ '' + (item.text || item.raw) + '' }}
                </view>
            </block>
        </block>
